...
We are greatly interested in crawling and scraping as many questions, as we can, from stack-exchange. This is an example of a question library page 
from stack-exchange.

Your task will be, to scrape the questions from each library page, in the order in which they are listed. You will be provided with the markup of question 
listing pages, from which you need to detect:
(1) Identifier (2) Question text (which is on the Hyperlink to the question) (3) How long ago the question was asked.

The Markup in the Test Cases will be similar to the sample fragment shown below. Please note, that since this markup is real markup from the website, it is
likely to contain some stray control and escape characters, unexpected whitespaces and newlines.
...
The output file should contain N lines, where N is the number of questions you have identified in the provided fragment.Each line contains the identifier, 
question text and (relative) time when the question was asked (with no leading or trailing spaces surrounding each section); separated by semi-colons. 
The information about the questions in the output file should match with the ordering in the original markup.
...
The first question has ID 80407, it is "about power supply of operational amplifier" and it was asked "11 hours ago" (relative to the time when this markup was
noted). Search for these values in the given markup fragment to gain a better understanding of where we identified these values from. The second question has
ID 80405, it is about "5V Regulator Power Dissipation", and it was asked "11 hours ago" (relative to the time when this markup was noted).
...
results = re.findall('(?<=\"question-summary-)\d+(?=\">)|(?<=\"question-hyperlink\">).+(?=<\/a)|(?<=\"relativetime\">).+(?=<\/span>)', stack)#, re.DOTALL)

for i in range(0,len(results),3):
    print(';'.join(results[i:i+3]))

# SOLUTION BY (Very good usage of groups with re.DOTALL
import re, sys

code = sys.stdin.read()

idTxtTime = re.findall(r'question-summary-(\d+?)\">.*?class=\"question-hyperlink\">(.+?)<\/a>.*?class=\"relativetime\">(.+?)<\/span>', code, re.DOTALL)

for found in idTxtTime:
    if len(found) == 3:
        print(";".join(found))
    else:
        continue
